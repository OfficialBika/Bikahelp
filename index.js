...
bot.on("new_chat_members", async (ctx) => {
  ...
  if (settings.approvalRequired && group?.status === "pending") {
    try {
      await bot.telegram.sendMessage(
        OWNER_ID,
...
Reject:
 /reject ${ctx.chat.id}`
  ...
  // Welcome message if cleanServices is OFF and custom welcome exists
  const members = ctx.message.new_chat_members || [];
  const me = members.find((x) => x.id === (ctx.botInfo?.id || 0));
  if (!me) return;

  if (!group?.settings?.cleanServicesEnabled) {
    const lang = "my";
    const welcome = group?.settings?.welcomeTextMy || (lang === "en" ? settings.welcomeTextEn : settings.welcomeTextMy);
    if (welcome) {
      try { await ctx.reply(welcome); } catch {}
    }
  }
});
