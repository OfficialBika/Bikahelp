...
async function buildWelcomeImageBuffer(fullName, groupTitle) {
  const safeName = escapeSvgText(fullName || "User");
  const safeGroup = escapeSvgText(groupTitle || "Our Group");
  const svg = `
  <svg width="1280" height="720" viewBox="0 0 1280 720" xmlns="http://www.w3.org/2000/svg">
    ...
    <circle cx="180" cy="130" r="120" fill="#22c55e" fill-opacity="0.12"/>
    <circle cx="1100" cy="580" r="170" fill="#06b6d4" fill-opacity="0.10"/>
    <rect x="70" y="70" rx="36" ry="36" width="1140" height="580" fill="url(#card)" stroke="#ffffff" stroke-opacity="0.14" filter="url(#shadow)"/>
    <text x="640" y="185" text-anchor="middle" font-size="44" font-weight="700" fill="#f8fafc">မင်္ဂလာပါ ${safeName}</text>
    <text x="640" y="270" text-anchor="middle" font-size="34" font-weight="600" fill="#e5e7eb">${safeGroup} မှ</text>
    <text x="640" y="340" text-anchor="middle" font-size="38" font-weight="700" fill="#fef3c7">လှိုက်လှဲစွာ ကြိုဆိုပါတယ်ဗျ</text>
    <rect x="235" y="420" rx="18" ry="18" width="810" height="86" fill="#ffffff" fill-opacity="0.08"/>
    <text x="640" y="475" text-anchor="middle" font-size="28" font-weight="500" fill="#e2e8f0">Welcome to the group • Be friendly • Enjoy your stay</text>
    <text x="640" y="600" text-anchor="middle" font-size="24" font-weight="500" fill="#cbd5e1">Powered by Bika Help Bot</text>
  </svg>`;
...
bot.command("search", async (ctx) => {
  ...
  if (!faq.length && !saved.length && !commands.length) return ctx.reply(t(access.lang, "noResult"));
...
bot.command("notes", async (ctx) => {
  ...
  await ctx.reply("🗂 Saved Keywords\n\n" + rows.map((x) => `• ${x.keyword} (${x.type})`).join("\n"));
});
...
bot.command("addfaq", async (ctx) => {
  ...
  if (parts.length < 2) return ctx.reply("Usage: /addfaq question | answer");
...
bot.command("editfaq", async (ctx) => {
  ...
  const parts = raw.split("|").map((x) => x.trim());
  if (parts.length < 3) return ctx.reply("Usage: /editfaq FAQ_ID | question | answer");
  const [id, q, a] = parts;
...
bot.command("save", async (ctx) => {
  ...
  const parts = raw.split("|").map((x) => x.trim());
  if (parts.length < 2) return ctx.reply("Usage: /save keyword | text");
...
bot.on("new_chat_members", async (ctx) => {
  ...
  const botSelf = members.find((x) => x.id === (ctx.botInfo?.id || 0));

  if (botSelf && settings.approvalRequired && group?.status === "pending") {
    try {
      await bot.telegram.sendMessage(
        OWNER_ID,
...
Reject:
 /reject ${ctx.chat.id}`
  ...
  const humanMembers = members.filter((x) => x.id !== (ctx.botInfo?.id || 0) && !x.is_bot);
  if (!humanMembers.length) return;

  if (!group?.settings?.welcomeEnabled) return;

  for (const member of humanMembers) {
    const fullName = `${member.first_name || ""} ${member.last_name || ""}`.trim() || member.username || "User";
    const mentionHtml = `<a href="tg://user?id=${member.id}">${escapeHtml(fullName)}</a>`;
    const customWelcome = group?.settings?.welcomeTextMy?.trim();
    const text = customWelcome || buildDefaultWelcomeText(fullName, mentionHtml, ctx.chat.title || group?.title || "ဒီ Group");

    if (group?.settings?.welcomePhotoEnabled) {
      try {
        const photoBuffer = await buildWelcomeImageBuffer(fullName, ctx.chat.title || group?.title || "Our Group");
        await ctx.replyWithPhoto({ source: photoBuffer, filename: "welcome.png" }, {
          caption: text,
          parse_mode: "HTML",
        });
        continue;
    ...
    if (group?.settings?.welcomeTextEnabled) {
      try {
