...
const START_IMG = String(process.env.START_IMG || "").trim();
const CHANNEL_USERNAME = String(process.env.CHANNEL_USERNAME || "").replace(/^@/, "").trim();
const OWNER_USERNAME = String(process.env.OWNER_USERNAME || "").replace(/^@/, "").trim();
...
async function buildGoodbyeImage(fullName, groupName, avatarBuffer = null) {
  const fs = require("fs");
  const os = require("os");
  const path = require("path");
  const { execFile } = require("child_process");
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "goodbye-"));
  const templateFile = path.join(process.cwd(), "assets", "welcome_template.png");
  ...
  const lineFile = path.join(tmpDir, "line.png");
  const footerFile = path.join(tmpDir, "footer.png");
  const outFile = path.join(tmpDir, "goodbye.png");
  ...
  await execFileAsync("pango-view", [
    ...
    "--margin=0",
    "--output", lineFile,
    "--text", "နောက်တစ်ခါ ပြန်တွေ့ကြမယ်နော်"
  ]);
...
function buildDefaultGoodbyeText(mentionHtml, groupTitle) {
  return `နှုတ်ဆက်ပါတယ် ${mentionHtml} ရေ

${escapeHtml(groupTitle || "ဒီ Group")} မှ ထွက်ခွာသွားပါပြီ

နောက်တစ်ခါ ပြန်တွေ့ကြမယ်နော်`;
}

function buildClickableMention(user) {
  const fullName = [user?.first_name, user?.last_name].filter(Boolean).join(" ").trim() || user?.first_name || "User";
...
function cooldownKey(userId, action) {
  return `${userId}:${action}`;
}

function getCooldownRemaining(userId, action, seconds) {
  const key = cooldownKey(userId, action);
  const last = cooldownStore.get(key) || 0;
  const nowMs = Date.now();
  const diff = nowMs - last;
  const limit = seconds * 1000;
  if (diff < limit) return Math.ceil((limit - diff) / 1000);
...
ဒီ bot က Powerful Help Bot ဖြစ်ပြီး FAQ, Search, Support Tickets, Saved Contents, Banwords, Clean Services, Admin System, Multi-Language, Tools စတဲ့ system တွေအများကြီး ပါဝင်ပါတယ်။`,
...
This is a Powerful Help Bot with FAQ, Search, Support Tickets, Saved Contents, Banwords, Clean Services, Admin System, Multi-Language, and useful tools.`,
...
function mainMenu(lang = "my") {
  if (lang === "en") {
    return Markup.inlineKeyboard([
      [Markup.button.callback("❓ Help", "menu:help"), Markup.button.callback("⌨️ Commands", "menu:commands")],
      [Markup.button.callback("📚 FAQ", "menu:faq"), Markup.button.callback("🔎 Search", "menu:search")],
      [Markup.button.callback("🎫 Support", "menu:support"), Markup.button.callback("🧰 Tools", "menu:tools")],
      [Markup.button.callback("🗂 Saved", "menu:saved"), Markup.button.callback("🌐 Language", "menu:lang")],
      [Markup.button.callback("ℹ️ About", "menu:about"), Markup.button.callback("📞 Contact Admin", "menu:contact")],
    ]);
  }
  return Markup.inlineKeyboard([
    [Markup.button.callback("❓ အကူအညီ", "menu:help"), Markup.button.callback("⌨️ Commands", "menu:commands")],
    [Markup.button.callback("📚 FAQ", "menu:faq"), Markup.button.callback("🔎 ရှာဖွေရန်", "menu:search")],
    [Markup.button.callback("🎫 ဆက်သွယ်ရန်", "menu:support"), Markup.button.callback("🧰 Tools", "menu:tools")],
    [Markup.button.callback("🗂 Saved", "menu:saved"), Markup.button.callback("🌐 ဘာသာစကား", "menu:lang")],
    [Markup.button.callback("ℹ️ အကြောင်း", "menu:about"), Markup.button.callback("📞 Admin ဆက်သွယ်ရန်", "menu:contact")],
  ]);
}
...
function startText(lang = "my", fullName = "User") {
  const safeName = escapeHtml(fullName);
  return lang === "en"
    ? `Hello ${safeName} 👋

This bot is a very important Help Bot for Telegram Group Chats.`
    : `Hello ${safeName} 👋

ဒီ bot က Telegram Group Chat တွေအတွက် အရမ်းကို အရေးပါတဲ့ Help Bot တစ်ခုပါ`;
}

function startButtons(lang = "my") {
  const helpLabel = lang === "en" ? "📘 Help" : "📘 Help";
  const channelLabel = lang === "en" ? "📢 Support Channel" : "📢 Support Channel";
  const addLabel = lang === "en" ? "➕ Add me your group" : "➕ Add me your group";
  const ownerLabel = lang === "en" ? "👤 Bot Owner" : "👤 Bot Owner";

  const rows = [
    [
      Markup.button.callback(helpLabel, "help:main"),
      CHANNEL_USERNAME
        ? Markup.button.url(channelLabel, `https://t.me/${CHANNEL_USERNAME}`)
        : Markup.button.callback(channelLabel, "start:nochannel")
    ],
    [
      BOT_USERNAME
        ? Markup.button.url(addLabel, `https://t.me/${BOT_USERNAME}?startgroup=true`)
        : Markup.button.callback(addLabel, "start:nobotusername"),
      OWNER_USERNAME
        ? Markup.button.url(ownerLabel, `https://t.me/${OWNER_USERNAME}`)
        : Markup.button.callback(ownerLabel, "start:noowner")
    ],
  ];
  return Markup.inlineKeyboard(rows);
}

function helpHomeText(lang = "my") {
  return lang === "en"
    ? `📘 Help Center

Choose a category below to see the commands and usage for that section.`
    : `📘 Help Center

အောက်က category တွေထဲက တစ်ခုကိုရွေးပြီး အဲဒီအပိုင်းနဲ့သက်ဆိုင်တဲ့ command နဲ့ usage တွေကိုကြည့်နိုင်ပါတယ်။`;
}

function helpMenu(lang = "my") {
  return Markup.inlineKeyboard([
    [
      Markup.button.callback(lang === "en" ? "👮 Admin" : "👮 Admin", "help:admin"),
      Markup.button.callback(lang === "en" ? "🚫 Banwords" : "🚫 Banwords", "help:banwords"),
      Markup.button.callback(lang === "en" ? "🔁 Filters" : "🔁 Filters", "help:filters")
    ],
    [
      Markup.button.callback(lang === "en" ? "👋 Greetings" : "👋 Greetings", "help:greetings"),
      Markup.button.callback(lang === "en" ? "🧹 Clean Service" : "🧹 Clean Service", "help:cleanservice"),
      Markup.button.callback(lang === "en" ? "🎫 Tickets" : "🎫 Tickets", "help:tickets")
    ],
    [
      Markup.button.callback(lang === "en" ? "📚 FAQ" : "📚 FAQ", "help:faq"),
      Markup.button.callback(lang === "en" ? "🗂 Saved" : "🗂 Saved", "help:saved"),
      Markup.button.callback(lang === "en" ? "🛠 Tools" : "🛠 Tools", "help:tools")
    ],
    [
      Markup.button.callback(lang === "en" ? "🌐 Language" : "🌐 Language", "help:language"),
      Markup.button.callback(lang === "en" ? "⌨️ Commands" : "⌨️ Commands", "help:commands"),
      Markup.button.callback(lang === "en" ? "👑 Owner" : "👑 Owner", "help:owner")
    ],
    [
      Markup.button.callback(lang === "en" ? "ℹ️ About" : "ℹ️ About", "help:about"),
      Markup.button.callback(lang === "en" ? "❌ Close" : "❌ Close", "menu:close")
    ],
  ]);
}

function helpBack(lang = "my") {
  return Markup.inlineKeyboard([
    [Markup.button.callback(lang === "en" ? "⬅️ Back" : "⬅️ Back", "help:main")],
  ]);
}

function helpSectionKeyboard(section, lang = "my") {
  const rows = [];
  if (["banwords", "filters", "greetings", "cleanservice"].includes(section)) {
    rows.push([Markup.button.callback(lang === "en" ? "🧪 Example usage" : "🧪 Example usage", `help:${section}:examples`)]);
  }
  rows.push([Markup.button.callback(lang === "en" ? "⬅️ Back" : "⬅️ Back", "help:main")]);
  return Markup.inlineKeyboard(rows);
}

function helpTextAdmin(lang = "my") {
  return lang === "en"
    ? `👮 Admin

Use the admin system to manage groups, users, content, and moderation.

Main commands:
• /admin
• /addfaq question | answer
• /editfaq FAQ_ID | question | answer
• /delfaq FAQ_ID
• /faqlist
• /save keyword | text
...
Admin system ကိုသုံးပြီး group, user, content နဲ့ moderation ပိုင်းတွေကို စီမံနိုင်ပါတယ်။
...
function helpTextBanwords(lang = "my") {
  return lang === "en"
    ? `🚫 Banwords

Block banned words in groups and stop unwanted words before they spread.
...
Tips:
• Use banwords for blocked words only
• Use filters for auto replies
• Bot needs Delete Messages permission`
    : `🚫 Banwords

Group ထဲမှာ မလိုတဲ့ banned word တွေကို block လုပ်ဖို့ banword system ကိုသုံးပါ။
...
Tips:
• banword ကို blocklist အတွက်ပဲ သုံးပါ
• filter ကို auto reply အတွက် သုံးပါ
• Bot မှာ Delete Messages permission လိုပါတယ်`;
}


function helpTextFilters(lang = "my") {
  return lang === "en"
    ? `🔁 Filters

Filters make your chat more lively. When someone says a trigger word, the bot replies automatically.
...
• /filter (hi, hey, hello) Hello back!

Supports:
• text replies
• reply-to-media filters
• placeholders like {first}, {fullname}, {username}, {mention}, {chatname}`
    : `🔁 Filters

Filter system က auto reply ပုံစံပါ။ user တစ်ယောက် trigger word ပြောလိုက်ရင် bot က response ပြန်ပေးမယ်။
...
• text replies
• reply-to-media filters
• placeholders - {first}, {fullname}, {username}, {mention}, {chatname}`;
}


function helpTextGreetings(lang = "my") {
  return lang === "en"
    ? `👋 Greetings

Welcome and goodbye messages are used to greet members when they join or leave the group.
...
Placeholders:
• {first} • {fullname} • {username} • {mention} • {chatname}`
    : `👋 Greetings

Welcome နဲ့ Goodbye စနစ်က group ထဲ join/leave ဖြစ်တိုင်း greeting message ပို့ပေးမယ့် system ပါ။
...
function helpTextCleanService(lang = "my") {
  return lang === "en"
    ? `🧹 Clean Service

Automatically delete telegram service messages to keep your group clean.
...
Telegram service messages တွေကို auto ဖျက်ပေးပြီး group ကိုရှင်းသန့်နေစေမယ့် system ပါ။
...
function helpTextTickets(lang = "my") {
  return lang === "en"
    ? `🎫 Tickets

Support ticket system for private support.
...
Private support အတွက် support ticket system ပါ။
...
function helpTextFaq(lang = "my") {
  return lang === "en"
    ? `📚 FAQ

FAQ and search system.
...
function helpTextSaved(lang = "my") {
  return lang === "en"
    ? `🗂 Saved

Saved content and notes system.
...
Saved content နဲ့ notes system ပါ။
...
function helpTextTools(lang = "my") {
  return userToolsHelp(lang);
}

function helpTextLanguage(lang = "my") {
  return lang === "en"
    ? `🌐 Language
...
function helpTextCommands(lang = "my", role = "user") {
  return `⌨️ Commands\n\n${buildHelpText(role, lang)}`;
}

function helpTextOwner(lang = "my") {
  return ownerCommandList(lang);
}

function helpTextAbout(lang = "my") {
  return lang === "en"
    ? `ℹ️ About

Telegram Powerful Help Bot
For Telegram groups: FAQ, Search, Support Tickets, Saved Contents, Banwords, Filters, Greetings, Clean Services, Admin System, Multi-Language, and useful tools.`
    : `ℹ️ About

Telegram Powerful Help Bot
Telegram group တွေအတွက် FAQ, Search, Support Tickets, Saved Contents, Banwords, Filters, Greetings, Clean Services, Admin System, Multi-Language, နှင့် useful tools ပါဝင်ပါတယ်။`;
}
...
const groupSchema = new mongoose.Schema({
  ...
  settings: {
    ...
    goodbyePhotoEnabled: { type: Boolean, default: false },
    goodbyeTextEnabled: { type: Boolean, default: true },
    goodbyeTextMy: { type: String, default: "" },
    goodbyeTextEn: { type: String, default: "" },
    rulesText: { type: String, default: "" },
    banwordsEnabled: { type: Boolean, default: true },
...
const settingSchema = new mongoose.Schema({
  ...
  globalBanwordEnabled: { type: Boolean, default: true },
...
const filterReplySchema = new mongoose.Schema({
  groupId: { type: Number, index: true },
  triggers: { type: [String], default: [] },
  rawTrigger: String,
  type: { type: String, default: "text" }, // text/photo/video/document
  text: String,
  fileId: String,
  createdBy: Number,
}, { timestamps: true });
...
const Banword = mongoose.model("Banword", filterSchema, "banwords");
const FilterReply = mongoose.model("FilterReply", filterReplySchema, "filters");
...
function normalizeTriggerValue(s) {
  return String(s || "").trim().toLowerCase();
}

function parseTriggerInput(input) {
  const raw = String(input || "").trim();
  if (!raw) return [];
  if (raw.startsWith("(") && raw.includes(")")) {
    const inside = raw.slice(1, raw.indexOf(")"));
    return inside.split(",").map((x) => normalizeTriggerValue(x)).filter(Boolean);
  }
  return [normalizeTriggerValue(raw)];
}

function parseFilterCommandArgs(raw) {
  const s = String(raw || "").trim();
  if (!s) return { rawTrigger: "", triggers: [], replyText: "" };
  ...
  if (s.startsWith('"')) {
    ...
    if (end > 1) {
      triggerPart = s.slice(1, end);
      replyText = s.slice(end + 1).trim();
    }
  } else if (s.startsWith("(")) {
    const end = s.indexOf(")");
    if (end > 0) {
      triggerPart = s.slice(0, end + 1);
      replyText = s.slice(end + 1).trim();
    }
  }
  ...
  if (!triggerPart) {
    ...
    if (firstSpace === -1) {
      ...
      triggerPart = s.slice(0, firstSpace).trim();
      replyText = s.slice(firstSpace + 1).trim();
    }
  }

  const triggers = parseTriggerInput(triggerPart.replace(/^"(.*)"$/, "$1"));
  return {
    rawTrigger: triggerPart.replace(/^"(.*)"$/, "$1"),
    triggers,
    replyText,
  };
}
...
function applyPlaceholders(template, ctx) {
  const firstRaw = ctx.from?.first_name || "User";
  const fullRaw = [ctx.from?.first_name, ctx.from?.last_name].filter(Boolean).join(" ").trim() || firstRaw;
  const usernameRaw = ctx.from?.username ? `@${ctx.from.username}` : "-";
  const chatnameRaw = ctx.chat?.title || "Group";

  const first = escapeHtml(firstRaw);
  const full = escapeHtml(fullRaw);
  const username = escapeHtml(usernameRaw);
  const chatname = escapeHtml(chatnameRaw);

  const mention = ctx.from?.id
    ? `<a href="tg://user?id=${ctx.from.id}">${full}</a>`
    : full;
  ...
  return String(template || "")
    ...
    .replace(/\{chatname\}/gi, chatname);
}

async function sendFilterReply(ctx, row) {
  const lang = await getLang(await ensureUser(ctx.from));
  const caption = row.text ? applyPlaceholders(row.text, ctx) : undefined;

  if (row.type === "text") {
    return ctx.reply(applyPlaceholders(row.text || "", ctx), { parse_mode: "HTML" });
  }
  if (row.type === "photo" && row.fileId) {
    return ctx.replyWithPhoto(row.fileId, { caption, parse_mode: "HTML" });
  }
  if (row.type === "video" && row.fileId) {
    return ctx.replyWithVideo(row.fileId, { caption, parse_mode: "HTML" });
  }
  if (row.type === "document" && row.fileId) {
    return ctx.replyWithDocument(row.fileId, { caption, parse_mode: "HTML" });
  }
  return ctx.reply(lang === "en" ? "Filter reply is incomplete." : "Filter reply မပြည့်စုံသေးပါ။");
}

async function findMatchingFilterReply(groupId, messageText) {
  const rows = await FilterReply.find({ groupId }).sort({ createdAt: -1 }).limit(200);
  const text = String(messageText || "").toLowerCase();
  for (const row of rows) {
    const triggers = Array.isArray(row.triggers) ? row.triggers : [];
    if (triggers.some((t) => t && text.includes(String(t).toLowerCase()))) {
      return row;
    }
  }
  return null;
...
async function applyBanwords(ctx) {
  ...
  if (!group?.settings?.banwordsEnabled) return false;

  const settings = await getSettings();
  const banwords = [];
  if (settings.globalBanwordEnabled) {
    const globals = await Banword.find({ scope: "global", enabled: true });
    banwords.push(...globals);
  }
  const locals = await Banword.find({ scope: "group", groupId: ctx.chat.id, enabled: true });
  banwords.push(...locals);

  const msgText = ctx.message.text || "";
  for (const f of banwords) {
    ...
    if (["warn", "delete_warn"].includes(f.actionType)) {
      ...
      try {
        await ctx.reply(`⚠️ ${ctx.from.first_name || "User"}, message blocked by banword.`);
      } catch {}
    }

    await writeLog("banword", ctx.from.id, ctx.from.first_name || "", "hit_banword", `${f.scope}:${f.pattern}`, ctx.from.id, "user", ctx.chat.id);
    return true;
  }
  return false;
}
...
function commandsMenu(lang = "my") {
  return Markup.inlineKeyboard([
    [Markup.button.callback(lang === "en" ? "👤 User Commands" : "👤 User Commands", "commands:user"),
     Markup.button.callback(lang === "en" ? "👥 Group Commands" : "👥 Group Commands", "commands:group")],
    [Markup.button.callback(lang === "en" ? "🛠 Admin Commands" : "🛠 Admin Commands", "commands:admin"),
     Markup.button.callback(lang === "en" ? "👑 Owner Commands" : "👑 Owner Commands", "commands:owner")],
    [Markup.button.callback(lang === "en" ? "⬅️ Back" : "⬅️ နောက်သို့", "menu:main"),
     Markup.button.callback(lang === "en" ? "❌ Close" : "❌ ပိတ်မည်", "menu:close")],
  ]);
}

function userCommandList(lang = "my") {
  return lang === "en"
    ? `👤 User Commands
...
/ticket [category] - Open ticket
/mytickets - My tickets
/closeticket ID - Close ticket
/replyticket ID text - Reply ticket
/get keyword - Get saved content
/notes - Saved keywords
/saved - Saved guide
/lang my
/lang en
...
/ticket [category] - Ticket ဖွင့်ရန်
/mytickets - ကိုယ့် tickets
/closeticket ID - Ticket ပိတ်ရန်
/replyticket ID text - Ticket reply
/get keyword - Saved content ထုတ်ရန်
/notes - Saved keywords
/saved - Saved guide
/lang my
/lang en
...
function groupCommandList(lang = "my") {
  return lang === "en"
    ? `👥 Group Commands
...
function adminCommandList(lang = "my") {
  return lang === "en"
    ? `🛠 Admin Commands
...
function ownerCommandList(lang = "my") {
  return lang === "en"
    ? `👑 Owner Commands
...
function commandsAccessDenied(lang = "my") {
  return lang === "en"
    ? "You do not have permission to view this command category."
    : "ဒီ command category ကိုကြည့်ရန် permission မရှိပါ။";
}

function buildHelpText(role = "user", lang = "my") {
  let sections = [userCommandList(lang), groupCommandList(lang)];
  if (role === "admin" || role === "owner") sections.push(adminCommandList(lang));
  if (role === "owner") sections.push(ownerCommandList(lang));
...
bot.use(async (ctx, next) => {
  try {
    ...
    if (ctx.message) {
      ...
      const blocked = await applyBanwords(ctx);
      if (blocked) return;
    }
    return next();
  } catch (err) {
...
bot.start(async (ctx) => {
  ...
  const fullName = [ctx.from.first_name, ctx.from.last_name].filter(Boolean).join(" ").trim() || ctx.from.first_name || "User";
  const caption = startText(lang, fullName);
  try {
    if (START_IMG) {
      await ctx.replyWithPhoto(START_IMG, { caption, parse_mode: "HTML", ...startButtons(lang) });
    } else {
      await ctx.replyWithHTML(caption, startButtons(lang));
    }
  } catch (e) {
    await ctx.replyWithHTML(caption, startButtons(lang));
  }
...
bot.command("help", async (ctx) => {
  const access = await accessGuard(ctx);
  if (!access.ok) return;
  await ctx.reply(helpHomeText(access.lang), helpMenu(access.lang));
});

bot.command("commands", async (ctx) => {
  const access = await accessGuard(ctx);
  if (!access.ok) return;
  const title = access.lang === "en" ? "⌨️ Command Menu" : "⌨️ Command Menu";
  await ctx.reply(title, commandsMenu(access.lang));
});
...
Banwords: ${group?.settings?.banwordsEnabled ? "ON" : "OFF"}
...
bot.on("text", async (ctx, next) => {
  ...
  // chat filters auto reply
  if (isGroup(ctx)) {
    try {
      const filterRow = await findMatchingFilterReply(ctx.chat.id, ctx.message.text);
      if (filterRow) {
        await sendFilterReply(ctx, filterRow);
        return;
      }
    } catch (e) {
      console.error("filter auto reply error:", e.message);
    }
  }
...
bot.command("filter", async (ctx) => {
  ...
  const raw = cleanCmd(ctx.message.text, "filter");
  if (!raw) {
    return ctx.reply(`Usage:
/filter hello Hello there!
/filter "hello friend" Hello back!
/filter (hi, hey, hello) Hello back!
Reply text/media with:
/filter trigger`);
  }

  const { rawTrigger, triggers, replyText } = parseFilterCommandArgs(raw);
  if (!triggers.length) {
    return ctx.reply("Invalid trigger.\nExample:\n/filter hello Hello there!");
  }

  let type = "text";
  let textValue = replyText;
  let fileId = "";
  ...
  if (reply) {
    if (reply.photo?.length) {
      ...
      textValue = replyText || reply.caption || "";
      ...
      type = "text";
      textValue = reply.text;
    }
  }

  if (type === "text" && !String(textValue || "").trim()) {
    return ctx.reply(`Usage:
/filter hello Hello there!
or reply to a text/media message with:
/filter hello`);
  }

  await FilterReply.findOneAndUpdate(
    { groupId: ctx.chat.id, rawTrigger: rawTrigger },
    {
      $set: {
        groupId: ctx.chat.id,
        rawTrigger,
  ...
  await writeLog("filter", ctx.from.id, ctx.from.first_name || "", "set_filter", rawTrigger, rawTrigger, "filter", ctx.chat.id);
  await ctx.reply(`Filter saved.\nTrigger: ${rawTrigger}\nType: ${type}`);
});
...
bot.command("filters", async (ctx) => {
  ...
  const rows = await FilterReply.find({ groupId: ctx.chat.id }).sort({ createdAt: -1 }).limit(100);
  if (!rows.length) return ctx.reply("No filters.");

  const out = rows.map((x, i) => `${i + 1}. ${x.rawTrigger} → ${x.type}`).join("\n");
  await ctx.reply(`📋 Chat Filters\n\n${out}`);
});
...
bot.command("stop", async (ctx) => {
  ...
  const raw = cleanCmd(ctx.message.text, "stop");
  if (!raw) return ctx.reply('Usage: /stop trigger\nExample: /stop hello\n/stop "hello friend"');

  const target = raw.replace(/^"(.*)"$/, "$1").trim();
  const result = await FilterReply.deleteOne({
    groupId: ctx.chat.id,
    $or: [{ rawTrigger: raw }, { rawTrigger: target }],
  });

  if (!result.deletedCount) return ctx.reply("Filter not found.");
  await writeLog("filter", ctx.from.id, ctx.from.first_name || "", "stop_filter", target, target, "filter", ctx.chat.id);
  await ctx.reply(`Filter removed: ${target}`);
});
...
bot.command("stopall", async (ctx) => {
  ...
  const result = await FilterReply.deleteMany({ groupId: ctx.chat.id });
  await writeLog("filter", ctx.from.id, ctx.from.first_name || "", "stopall_filters", String(result.deletedCount), "", "filter", ctx.chat.id);
  await ctx.reply(`Removed ${result.deletedCount} filters.`);
});
...
bot.command("ticket", async (ctx) => {
  ...
  const remaining = getCooldownRemaining(ctx.from.id, "ticket_create", 30);
  if (remaining > 0) {
    return ctx.reply(access.lang === "en"
      ? `Please wait ${remaining}s before opening another ticket.`
      : `Ticket အသစ်ဖွင့်ရန် ${remaining} စက္ကန့် စောင့်ပါ။`);
  }
  ...
  ctx.session.awaitingTicketCategory = category;
  await ctx.reply((access.lang === "en" ? `Category: ${category}

` : `Category: ${category}

`) + t(access.lang, "ticketAsk"));
});
...
bot.command("mytickets", async (ctx) => {
  ...
  const out = rows.map((x) => `• ${x.ticketId} — ${x.status} — [${x.category || "general"}] — ${formatDate(x.updatedAt)}`).join("\n");
...
bot.command("closeticket", async (ctx) => {
  ...
  ticket.closedAt = now();
  await ticket.save();
  await writeLog("ticket", ctx.from.id, ctx.from.first_name || "", "ticket_close_user", ticket.ticketId, ticket.ticketId, "ticket", ctx.chat.id);
...
bot.command("replyticket", async (ctx) => {
  ...
  await ticket.save();
  await writeLog("ticket", ctx.from.id, ctx.from.first_name || "", "ticket_reply_user", ticket.ticketId, ticket.ticketId, "ticket", ctx.chat.id);
...
Banwords: ${g.settings.banwordsEnabled ? "ON" : "OFF"}
...
bot.command("banword", async (ctx) => {
  ...
  if (sub === "add") {
    ...
    if (!word) return ctx.reply("Usage: /banword add word");
    await Banword.findOneAndUpdate(
    ...
    return ctx.reply("Group banword added.");
  }

  if (sub === "del") {
    const word = args.slice(1).join(" ").trim().toLowerCase();
    if (!word) return ctx.reply("Usage: /banword del word");
    await Banword.deleteOne({ scope: "group", groupId: ctx.chat.id, pattern: word });
    return ctx.reply("Group banword removed.");
  }

  if (sub === "on" || sub === "off") {
    await Group.updateOne({ groupId: ctx.chat.id }, { $set: { "settings.banwordsEnabled": sub === "on" } });
    return ctx.reply(`Group banwords ${sub.toUpperCase()}`);
  }
...
bot.command("banwords", async (ctx) => {
  ...
  if (!access.ok) return;
  if (!isGroup(ctx)) return ctx.reply(t(access.lang, "onlyGroup"));
  const rows = await Banword.find({ scope: "group", groupId: ctx.chat.id }).sort({ createdAt: -1 });
  if (!rows.length) return ctx.reply("No group banwords.");
  await ctx.reply("Group Banwords\n\n" + rows.map((x) => `• ${x.pattern} (${x.matchType}/${x.actionType})`).join("\n"));
});

bot.command("gbanword", async (ctx) => {
  const access = await accessGuard(ctx);
  if (!access.ok) return;
  ...
  if (sub === "add") {
    const word = args.slice(1).join(" ").trim().toLowerCase();
    if (!word) return ctx.reply("Usage: /gbanword add word");
    await Banword.findOneAndUpdate(
      { scope: "global", pattern: word },
      { $set: { matchType: "partial", actionType: "delete_warn", enabled: true, createdBy: ctx.from.id } },
      { upsert: true, new: true }
    );
    return ctx.reply("Global banword added.");
  }

  if (sub === "del") {
    const word = args.slice(1).join(" ").trim().toLowerCase();
    if (!word) return ctx.reply("Usage: /gbanword del word");
    await Banword.deleteOne({ scope: "global", pattern: word });
    return ctx.reply("Global banword removed.");
  }

  if (sub === "on" || sub === "off") {
    await Setting.updateOne({}, { $set: { globalBanwordEnabled: sub === "on" } });
    return ctx.reply(`Global banword ${sub.toUpperCase()}`);
  }

  return ctx.reply("Usage:\n/gbanword add word\n/gbanword del word\n/gbanword on\n/gbanword off");
});

bot.command("gbanwords", async (ctx) => {
  const access = await accessGuard(ctx);
  if (!access.ok) return;
  if (!(await isOwner(ctx.from.id))) return ctx.reply(t(access.lang, "noPerm"));
  const rows = await Banword.find({ scope: "global" }).sort({ createdAt: -1 });
  if (!rows.length) return ctx.reply("No global banwords.");
  await ctx.reply("Global Banwords\n\n" + rows.map((x) => `• ${x.pattern} (${x.matchType}/${x.actionType})`).join("\n"));
});
...
bot.command("setgoodbye", async (ctx) => {
  ...
  if (!access.ok) return;
  if (!(await requireGroupAdminOrHigher(ctx, access.lang))) return;
  const textValue = cleanCmd(ctx.message.text, "setgoodbye");
  if (!textValue) return ctx.reply("Usage: /setgoodbye your custom goodbye text");
  await Group.updateOne(
    { groupId: ctx.chat.id },
    { $set: { "settings.goodbyeTextMy": textValue, "settings.goodbyeEnabled": true, "settings.goodbyeTextEnabled": true } }
  );
  await ctx.reply("Custom goodbye text saved.");
});

bot.command("testgoodbye", async (ctx) => {
  try {
    if (!["group", "supergroup"].includes(ctx.chat?.type)) return ctx.reply("Group only");
    const group = await ensureGroup(ctx.chat);
    const fullName = [ctx.from.first_name, ctx.from.last_name].filter(Boolean).join(" ").trim() || "User";
    const mention = buildClickableMention(ctx.from);
    const groupName = ctx.chat.title || "Group";
    let textValue = (group?.settings?.goodbyeTextMy || "").trim();
    if (textValue) {
      textValue = textValue
        .replace(/\{first\}/gi, escapeHtml(ctx.from.first_name || "User"))
        .replace(/\{fullname\}/gi, escapeHtml(fullName))
        .replace(/\{username\}/gi, ctx.from.username ? `@${escapeHtml(ctx.from.username)}` : "-")
        .replace(/\{mention\}/gi, mention)
        .replace(/\{chatname\}/gi, escapeHtml(groupName));
    } else {
      textValue = buildDefaultGoodbyeText(mention, groupName);
    }

    if (group?.settings?.goodbyePhotoEnabled && typeof buildGoodbyeImage === "function") {
      try {
        const avatarBuffer = await getUserAvatarBuffer(ctx.from.id);
        const buffer = await buildGoodbyeImage(fullName, groupName, avatarBuffer);
        await ctx.replyWithPhoto({ source: buffer }, { caption: textValue, parse_mode: "HTML" });
        return;
      } catch (e) {
        console.log("[TESTGOODBYE] photo failed:", e?.message || e);
      }
    }
    await ctx.replyWithHTML(textValue);
  } catch (err) {
    console.error("testgoodbye error:", err);
    await ctx.reply("Test goodbye failed.");
  }
});

bot.command("goodbyestatus", async (ctx) => {
  const access = await accessGuard(ctx);
  if (!access.ok) return;
  if (!isGroup(ctx)) return ctx.reply(t(access.lang, "onlyGroup"));
  const g = await Group.findOne({ groupId: ctx.chat.id });
...
Enabled: ${g.settings.goodbyeEnabled ? "ON" : "OFF"}
Photo: ${g.settings.goodbyePhotoEnabled ? "ON" : "OFF"}
Text: ${g.settings.goodbyeTextEnabled ? "ON" : "OFF"}
Custom Text: ${g.settings.goodbyeTextMy ? "YES" : "DEFAULT"}`
  );
});
...
bot.command("goodbye", async (ctx) => {
  ...
  if (!(await requireGroupAdminOrHigher(ctx, access.lang))) return;
  const state = onOff(splitArgs(ctx.message.text).args[0]);
  await Group.updateOne({ groupId: ctx.chat.id }, { $set: { "settings.goodbyeEnabled": state, "settings.greetingEnabled": true } });
  await ctx.reply(`Goodbye ${state ? "ON" : "OFF"}`);
});

bot.command("goodbyephoto", async (ctx) => {
  const access = await accessGuard(ctx);
  if (!access.ok) return;
  if (!(await requireGroupAdminOrHigher(ctx, access.lang))) return;
  const state = onOff(splitArgs(ctx.message.text).args[0]);
  await Group.updateOne({ groupId: ctx.chat.id }, { $set: { "settings.goodbyePhotoEnabled": state, "settings.goodbyeEnabled": true, "settings.greetingEnabled": true } });
  await ctx.reply(`Goodbye Photo ${state ? "ON" : "OFF"}`);
});

bot.command("goodbyetext", async (ctx) => {
  const access = await accessGuard(ctx);
  if (!access.ok) return;
  if (!(await requireGroupAdminOrHigher(ctx, access.lang))) return;
  const state = onOff(splitArgs(ctx.message.text).args[0]);
  await Group.updateOne({ groupId: ctx.chat.id }, { $set: { "settings.goodbyeTextEnabled": state, "settings.goodbyeEnabled": true, "settings.greetingEnabled": true } });
  await ctx.reply(`Goodbye Text ${state ? "ON" : "OFF"}`);
});

bot.command("greetingstatus", async (ctx) => {
  const access = await accessGuard(ctx);
  if (!access.ok) return;
  if (!isGroup(ctx)) return ctx.reply(t(access.lang, "onlyGroup"));
  const g = await Group.findOne({ groupId: ctx.chat.id });
...
Greeting: ${g.settings.greetingEnabled !== false ? "ON" : "OFF"}
Welcome: ${g.settings.welcomeEnabled ? "ON" : "OFF"}
Welcome Photo: ${g.settings.welcomePhotoEnabled ? "ON" : "OFF"}
Welcome Text: ${g.settings.welcomeTextEnabled ? "ON" : "OFF"}
Goodbye: ${g.settings.goodbyeEnabled ? "ON" : "OFF"}
Goodbye Photo: ${g.settings.goodbyePhotoEnabled ? "ON" : "OFF"}
Goodbye Text: ${g.settings.goodbyeTextEnabled ? "ON" : "OFF"}`
  );
});
...
bot.command("greeting", async (ctx) => {
  ...
  await Group.updateOne(
    { groupId: ctx.chat.id },
    { $set: { "settings.greetingEnabled": state, "settings.welcomeEnabled": state, "settings.goodbyeEnabled": state } }
  );
  await ctx.reply(`Greeting ${state ? "ON" : "OFF"}`);
});
...
bot.command("welcome", async (ctx) => {
  ...
  if (!(await requireGroupAdminOrHigher(ctx, access.lang))) return;
  const state = onOff(splitArgs(ctx.message.text).args[0]);
  await Group.updateOne({ groupId: ctx.chat.id }, { $set: { "settings.welcomeEnabled": state, "settings.greetingEnabled": true } });
...
bot.command("welcomephoto", async (ctx) => {
  ...
  if (!(await requireGroupAdminOrHigher(ctx, access.lang))) return;
  const state = onOff(splitArgs(ctx.message.text).args[0]);
  await Group.updateOne({ groupId: ctx.chat.id }, { $set: { "settings.welcomePhotoEnabled": state, "settings.welcomeEnabled": true, "settings.greetingEnabled": true } });
...
bot.command("welcometext", async (ctx) => {
  ...
  if (!(await requireGroupAdminOrHigher(ctx, access.lang))) return;
  const state = onOff(splitArgs(ctx.message.text).args[0]);
  await Group.updateOne({ groupId: ctx.chat.id }, { $set: { "settings.welcomeTextEnabled": state, "settings.welcomeEnabled": true, "settings.greetingEnabled": true } });
...
bot.command("tickets", async (ctx) => {
  ...
  await ctx.reply("Tickets\n\n" + rows.map((x) => `• ${x.ticketId} | ${x.status} | ${x.userId} | [${x.category || "general"}] | admin:${x.assignedAdmin || "-"}`).join("\n"));
});
...
bot.command("closedtickets", async (ctx) => {
  ...
  await ctx.reply("Closed Tickets\n\n" + rows.map((x) => `• ${x.ticketId} | ${x.userId} | [${x.category || "general"}] | admin:${x.assignedAdmin || "-"}`).join("\n"));
});
...
bot.command("ticketreply", async (ctx) => {
  ...
  await ticket.save();
  await writeLog("ticket", ctx.from.id, ctx.from.first_name || "", "ticket_reply_admin", ticket.ticketId, ticket.ticketId, "ticket", ctx.chat.id);
...
bot.command("closeticketadmin", async (ctx) => {
  ...
  ticket.status = "closed";
  ticket.closedAt = now();
  ticket.assignedAdmin = ticket.assignedAdmin || ctx.from.id;
  await ticket.save();
  await writeLog("ticket", ctx.from.id, ctx.from.first_name || "", "ticket_close_admin", ticket.ticketId, ticket.ticketId, "ticket", ctx.chat.id);
...
bot.command("reopenticket", async (ctx) => {
  ...
  ticket.assignedAdmin = ticket.assignedAdmin || ctx.from.id;
  await ticket.save();
  await writeLog("ticket", ctx.from.id, ctx.from.first_name || "", "ticket_reopen_admin", ticket.ticketId, ticket.ticketId, "ticket", ctx.chat.id);
...
bot.command("ban", async (ctx) => {
  ...
  await writeLog("admin", ctx.from.id, ctx.from.first_name || "", "ban_user", String(id), id, "user", ctx.chat.id);
...
bot.command("unban", async (ctx) => {
  ...
  await writeLog("admin", ctx.from.id, ctx.from.first_name || "", "unban_user", String(id), id, "user", ctx.chat.id);
...
bot.command("banwordlogs", async (ctx) => {
  const access = await accessGuard(ctx); if (!access.ok) return;
  if (!(await isAdmin(ctx.from.id))) return ctx.reply(t(access.lang, "noPerm"));
  const rows = await Log.find({ type: "banword" }).sort({ timestamp: -1 }).limit(20);
  if (!rows.length) return ctx.reply("No banword logs.");
  await ctx.reply(rows.map((x) => `• ${x.details} | user:${x.actorId} | ${formatDate(x.timestamp)}`).join("\n"));
});

bot.command("ticketlogs", async (ctx) => {
  const access = await accessGuard(ctx); if (!access.ok) return;
  if (!(await isAdmin(ctx.from.id))) return ctx.reply(t(access.lang, "noPerm"));
  const rows = await Log.find({ type: "ticket" }).sort({ timestamp: -1 }).limit(20);
  if (!rows.length) return ctx.reply("No ticket logs.");
  await ctx.reply(rows.map((x) => `• ${x.action} | ${x.details} | ${formatDate(x.timestamp)}`).join("\n"));
});

bot.command("banlogs", async (ctx) => {
  const access = await accessGuard(ctx); if (!access.ok) return;
  if (!(await isAdmin(ctx.from.id))) return ctx.reply(t(access.lang, "noPerm"));
  const rows = await Log.find({ action: { $in: ["ban_user", "unban_user"] } }).sort({ timestamp: -1 }).limit(20);
  if (!rows.length) return ctx.reply("No ban logs.");
  await ctx.reply(rows.map((x) => `• ${x.action} | target:${x.targetId} | by:${x.actorId} | ${formatDate(x.timestamp)}`).join("\n"));
});
...
async function doBroadcast(ctx, targetMode = "all") {
  ...
  const remain = getCooldownRemaining(ctx.from.id, `broadcast_${targetMode}`, 15);
  if (remain > 0) return ctx.reply(access.lang === "en" ? `Please wait ${remain}s before broadcasting again.` : `Broadcast ပြန်ပို့ရန် ${remain} စက္ကန့် စောင့်ပါ။`);
  const reply = ctx.message.reply_to_message;
...
Tickets: ${counts[4]}
Saved: ${counts[5]}
Banwords: ${counts[6]}\nFilters: ${await FilterReply.countDocuments()}
Logs: ${counts[7]}
Broadcasts: ${counts[8]}
CustomCmds: ${counts[9]}`
  );
...
Global Banword: ${settings.globalBanwordEnabled}
...
bot.use(async (ctx, next) => {
  ...
  const builtins = new Set([
    "start","help","commands","about","ping","uptime","lang","id","userinfo","groupinfo",
    ...
    "ghelp","groupsettings","approval","requestapprove","filter","filters","stop","stopall","banword","banwords","gbanword","gbanwords",
    "cleanservices","cleanjoin","cleanleave","cleanpin","cleanvoice","cleantitle","cleanphoto","cleanstatus",
    "warn","warnings","clearwarns",
    "admin","admins","addfaq","editfaq","delfaq","faqlist","save","editsave","delsave","savedlist","tickets","opentickets","closedtickets","ticketreply","closeticketadmin","reopenticket","ban","unban","user","finduser","groups","group","leavegroup","blacklistgroup","unblacklistgroup","stats","logs","errorlogs","banwordlogs","ticketlogs","banlogs","addcmd","editcmd","delcmd","cmdlist","broadcast","broadcastusers","broadcastgroups","broadcastall",
...
bot.action("menu:main", async (ctx) => {
  ...
  if (START_IMG) {
    try {
      return await ctx.editMessageMedia({ type: "photo", media: START_IMG, caption, parse_mode: "HTML" }, startButtons(lang));
    } catch {}
  }
  await ctx.editMessageText(caption, { parse_mode: "HTML", ...startButtons(lang) });
});
...
bot.action("menu:help", async (ctx) => {
  ...
  await ctx.reply(helpHomeText(lang), helpMenu(lang));
});
...
bot.action("help:main", async (ctx) => {
  ...
  await ctx.answerCbQuery().catch(() => {});
  try {
    await ctx.editMessageText(helpHomeText(lang), helpMenu(lang));
  } catch {
    await ctx.reply(helpHomeText(lang), helpMenu(lang));
  }
});

function helpExamplesBanwords(lang = "my") {
  return lang === "en"
    ? `🧪 Banwords Examples
...
function helpExamplesFilters(lang = "my") {
  return lang === "en"
    ? `🧪 Filter Examples
...
• /filter "hello friend" Hello back!
• /filter (hi, hey, hello) Hello back!
• reply to a photo then send: /filter promo
• /stop hello
• /stopall`
    : `🧪 Filter Examples
...
• /filter "hello friend" Hello back!
• /filter (hi, hey, hello) Hello back!
• photo ကို reply လုပ်ပြီး /filter promo
• /stop hello
• /stopall`;
}

function helpExamplesGreetings(lang = "my") {
  return lang === "en"
    ? `🧪 Greetings Examples
...
function helpExamplesCleanService(lang = "my") {
  return lang === "en"
    ? `🧪 Clean Service Examples
...
bot.action("help:admin", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  if (!(await isAdmin(ctx.from.id))) return ctx.editMessageText(commandsAccessDenied(lang), helpBack(lang));
  await ctx.editMessageText(helpTextAdmin(lang), helpBack(lang));
});

bot.action("help:banwords", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpTextBanwords(lang), helpSectionKeyboard("banwords", lang));
});

bot.action("help:filters", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpTextFilters(lang), helpSectionKeyboard("filters", lang));
});

bot.action("help:greetings", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpTextGreetings(lang), helpSectionKeyboard("greetings", lang));
});

bot.action("help:cleanservice", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpTextCleanService(lang), helpSectionKeyboard("cleanservice", lang));
});

bot.action("help:tickets", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpTextTickets(lang), helpBack(lang));
});

bot.action("help:faq", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpTextFaq(lang), helpBack(lang));
});

bot.action("help:saved", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpTextSaved(lang), helpBack(lang));
});

bot.action("help:tools", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpTextTools(lang), helpBack(lang));
});

bot.action("help:language", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpTextLanguage(lang), helpBack(lang));
});

bot.action("help:commands", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  const role = (await isOwner(ctx.from.id)) ? "owner" : ((await isAdmin(ctx.from.id)) ? "admin" : "user");
  await ctx.editMessageText(helpTextCommands(lang, role), helpBack(lang));
});

bot.action("help:owner", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  if (!(await isOwner(ctx.from.id))) return ctx.editMessageText(commandsAccessDenied(lang), helpBack(lang));
  await ctx.editMessageText(helpTextOwner(lang), helpBack(lang));
});

bot.action("help:about", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpTextAbout(lang), helpBack(lang));
});

bot.action("help:banwords:examples", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpExamplesBanwords(lang), helpSectionKeyboard("banwords", lang));
});

bot.action("help:filters:examples", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpExamplesFilters(lang), helpSectionKeyboard("filters", lang));
});

bot.action("help:greetings:examples", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpExamplesGreetings(lang), helpSectionKeyboard("greetings", lang));
});

bot.action("help:cleanservice:examples", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpExamplesCleanService(lang), helpSectionKeyboard("cleanservice", lang));
});

bot.action("start:nochannel", async (ctx) => {

  await ctx.answerCbQuery("Set CHANNEL_USERNAME in .env", { show_alert: true });
});

bot.action("start:nobotusername", async (ctx) => {
  await ctx.answerCbQuery("Set BOT_USERNAME in .env", { show_alert: true });
});

bot.action("start:noowner", async (ctx) => {
  await ctx.answerCbQuery("Set OWNER_USERNAME in .env", { show_alert: true });
});
...
bot.action("menu:commands", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(lang === "en" ? "⌨️ Command Menu" : "⌨️ Command Menu", commandsMenu(lang));
});

bot.action("commands:user", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(userCommandList(lang), backButtons(lang, "menu:commands"));
});

bot.action("commands:group", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(groupCommandList(lang), backButtons(lang, "menu:commands"));
});

bot.action("commands:admin", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  if (!(await isAdmin(ctx.from.id))) {
    return ctx.editMessageText(commandsAccessDenied(lang), backButtons(lang, "menu:commands"));
  }
  await ctx.editMessageText(adminCommandList(lang), backButtons(lang, "menu:commands"));
});

bot.action("commands:owner", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  if (!(await isOwner(ctx.from.id))) {
    return ctx.editMessageText(commandsAccessDenied(lang), backButtons(lang, "menu:commands"));
  }
  await ctx.editMessageText(ownerCommandList(lang), backButtons(lang, "menu:commands"));
});
...
Reject:
 /reject ${ctx.chat.id}`
      ...
      const greetingEnabled = group?.settings?.greetingEnabled !== false;
      const welcomeEnabled = greetingEnabled && (group?.settings?.welcomeEnabled !== false);
      ...
      for (const member of members) {
        ...
        const mention = buildClickableMention(member);
        const groupName = ctx.chat.title || "Group";

        let defaultText = (group?.settings?.welcomeTextMy || "").trim();
        if (defaultText) {
          defaultText = defaultText
            .replace(/\{first\}/gi, escapeHtml(member.first_name || "User"))
            .replace(/\{fullname\}/gi, escapeHtml(fullName))
            .replace(/\{username\}/gi, member.username ? `@${escapeHtml(member.username)}` : "-")
            .replace(/\{mention\}/gi, mention)
            .replace(/\{chatname\}/gi, escapeHtml(groupName));
        } else {
          defaultText = buildDefaultWelcomeText(fullName, mention, groupName);
        }
        ...
        if (welcomePhotoEnabled && typeof buildWelcomeImage === "function") {
          try {
            ...
            const buffer = await buildWelcomeImage(fullName, groupName, avatarBuffer);
            await ctx.replyWithPhoto({ source: buffer }, { caption: welcomeTextEnabled ? defaultText : undefined, parse_mode: "HTML" });
    ...
    if (ctx.message?.left_chat_member) {
      ...
      const greetingEnabled = group?.settings?.greetingEnabled !== false;
      const goodbyeEnabled = greetingEnabled && (group?.settings?.goodbyeEnabled === true);
      const goodbyePhotoEnabled = group?.settings?.goodbyePhotoEnabled === true;
      const goodbyeTextEnabled = group?.settings?.goodbyeTextEnabled !== false;
      ...
      if (goodbyePhotoEnabled && typeof buildGoodbyeImage === "function") {
        try {
          const avatarBuffer = await getUserAvatarBuffer(member.id);
          const buffer = await buildGoodbyeImage(fullName, groupName, avatarBuffer);
          await ctx.replyWithPhoto({ source: buffer }, { caption: goodbyeTextEnabled ? textValue : undefined, parse_mode: "HTML" });
          console.log("[GOODBYE] photo sent for:", fullName);
          return;
        } catch (err) {
          console.log("[GOODBYE] photo failed, fallback to text:", err?.message || err);
        }
      }

      if (goodbyeTextEnabled) {
        await ctx.replyWithHTML(textValue);
        console.log("[GOODBYE] text sent for:", fullName);
      }
      return;
    }
  } catch (err) {
