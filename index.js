...
bot.command("help", async (ctx) => {
  ...
  const title = access.lang === "en" ? "📘 Full Command List" : "📘 Full Command List";
  await ctx.reply(`${title}

${buildHelpText(role, access.lang)}`, backButtons(access.lang));
});
...
bot.action("menu:help", async (ctx) => {
  ...
  const lang = await getLang(user);
  const role = (await isOwner(ctx.from.id)) ? "owner" : ((await isAdmin(ctx.from.id)) ? "admin" : "user");
  await ctx.editMessageText(`📘 Full Command List\n\n${buildHelpText(role, lang)}`, backButtons(lang));
});
