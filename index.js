...
Banwords are used to block banned words in chats.
...
Banwords ကို group ထဲက ပိတ်ချင်တဲ့စာတွေ block လုပ်ဖို့ သုံးပါတယ်။
...
Filters are auto-reply triggers for your chat.
...
• /filter "hello friend" Welcome back!
• /filter (hi, hey, hello) Hello back!
• reply to a photo and send /filter promo
...
Filters က chat ထဲမှာ auto-reply trigger system ပါ။
...
Welcome and goodbye messages for join/left events.
...
Join/left event တွေအတွက် welcome နဲ့ goodbye messages ကို စီမံနိုင်ပါတယ်။
...
Clean unwanted service messages in groups.
...
Group ထဲက unwanted service messages တွေကို ဖျက်ရန် သုံးပါတယ်။
...
bot.action("help:banwords", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpTextBanwords(lang), helpBack(lang));
});

bot.action("help:filters", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpTextFilters(lang), helpBack(lang));
});

bot.action("help:greetings", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpTextGreetings(lang), helpBack(lang));
});

bot.action("help:cleanservice", async (ctx) => {
  const user = await ensureUser(ctx.from);
  const lang = await getLang(user);
  await ctx.editMessageText(helpTextCleanService(lang), helpBack(lang));
});
