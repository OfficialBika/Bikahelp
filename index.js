...
bot.command("goodbye", async (ctx) => {
  ...
  if (!(await requireGroupAdminOrHigher(ctx, access.lang))) return;
  const state = onOff(splitArgs(ctx.message.text).args[0]);
  await Group.updateOne({ groupId: ctx.chat.id }, { $set: { "settings.goodbyeEnabled": state } });
...
bot.command("goodbyephoto", async (ctx) => {
  ...
  if (!(await requireGroupAdminOrHigher(ctx, access.lang))) return;
  const state = onOff(splitArgs(ctx.message.text).args[0]);
  await Group.updateOne({ groupId: ctx.chat.id }, { $set: { "settings.goodbyePhotoEnabled": state, "settings.goodbyeEnabled": true } });
...
bot.command("goodbyetext", async (ctx) => {
  ...
  if (!(await requireGroupAdminOrHigher(ctx, access.lang))) return;
  const state = onOff(splitArgs(ctx.message.text).args[0]);
  await Group.updateOne({ groupId: ctx.chat.id }, { $set: { "settings.goodbyeTextEnabled": state, "settings.goodbyeEnabled": true } });
...
Reject:
 /reject ${ctx.chat.id}`
    ...
    if (ctx.message?.left_chat_member) {
      ...
      const goodbyeEnabled = group?.settings?.goodbyeEnabled === true;
